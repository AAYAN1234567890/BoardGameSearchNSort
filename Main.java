...
// Search: Print board games based on the number of players
private static void searchMaxPlayers(ArrayList<String> collection) {
    try {
        ...
        for (String game : collection) {
            ...
            int players = Integer.parseInt(details[3]);
            if (players <= maxPlayers) {
                filteredGames.add(game);
            }
        }

        System.out.println("\nGames with maximum players " + maxPlayers + " or less:");
        wait(2);
        filteredGames.forEach(Main::printFormattedBoardGame);
    } catch (NumberFormatException e) {
...
private static void saveToCSV(ArrayList<String> collection, String filename) {
    try (FileWriter writer = new FileWriter(filename)) {
        writer.write("Title,Rating,Difficulty,Players,Time,Year,Genre\n"); // Add header
        for (String game : collection) {
            writer.write(game + "\n");
        }
...
// Sorting: Selection sort by a specified index and data type
...
// Utility: Print a formatted board game entry
...
// Utility: Wait for a specified duration (in seconds)
